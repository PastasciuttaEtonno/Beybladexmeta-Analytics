...
async function run() {
  try {
    ...
    await db.execute(sql`
      ...
      SELECT 'blade' AS component_type, blade AS name, primi_posti, secondi_posti, terzi_posti, punteggio_totale, season
      FROM blade_stats
      UNION ALL
      SELECT 'assist-blade' AS component_type, assist_blade AS name, primi_posti, secondi_posti, terzi_posti, punteggio_totale, season
      FROM assist_blade_stats
      UNION ALL
      SELECT 'ratchet' AS component_type, ratchet AS name, primi_posti, secondi_posti, terzi_posti, punteggio_totale, season
      FROM ratchet_stats
      UNION ALL
      SELECT 'bit' AS component_type, bit AS name, primi_posti, secondi_posti, terzi_posti, punteggio_totale, season
      FROM bit_stats
      UNION ALL
      SELECT 'lock-chip' AS component_type, lock_chip AS name, primi_posti, secondi_posti, terzi_posti, punteggio_totale, season
    ...
    await db.execute(sql`CREATE UNIQUE INDEX IF NOT EXISTS top_component_snapshot_unique ON top_component_snapshot(component_type, name, season)`);
    try {
