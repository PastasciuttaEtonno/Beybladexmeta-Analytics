...
export async function generateComboImage(combo: any): Promise<Buffer> {
    ...
    try {
        // The backend owns a copy of the logo: it can no longer reach into the
        // frontend's public/ folder now that the two are separate services.
        const logoPath = process.env.OG_LOGO_PATH
            || path.join(process.cwd(), 'assets', 'meta-logo-white.png');
