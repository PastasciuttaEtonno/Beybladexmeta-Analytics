...
export function registerOgRoutes(app: Express): void {
  ...
  //
  // The frontend is a separate service now, so instead of reading index.html off
  // disk we fetch it from the frontend origin and rewrite the two image tags.
  // The document is cached briefly so a burst of shares doesn't hammer nginx.
  ...
  app.get('/combo/:id', async (req, res, next) => {
    try {
      ...
      const baseUrl = (process.env.APP_BASE_URL || 'https://beybladexmeta.com').replace(/\/$/, '');
      const ogImageUrl = `${baseUrl}/api/og/combo/${comboId}`;
