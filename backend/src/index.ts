...
// CORS.
// The recommended deployment keeps frontend and backend behind ONE public origin
// (the reverse proxy routes /api/* here), in which case no CORS is needed and
// CORS_ORIGINS stays empty. Set it to a comma-separated allowlist only when the
// frontend is served from a different origin — credentials are session cookies,
// so a wildcard origin is not an option.
...
if (corsOrigins.length > 0) {
  ...
  log(`CORS enabled for: ${corsOrigins.join(', ')}`);
}
...
(async () => {
  ...
  // This service no longer serves the SPA — the frontend is deployed separately.
  // Anything that reaches here is a genuine miss, so answer with JSON, not HTML.
  app.use((req, res) => {
    res.status(404).json({ error: "not_found", path: req.originalUrl });
  });

  // Serve on the port given by PORT (Coolify injects it); default 5000.
